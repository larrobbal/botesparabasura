<?php
    ...
    else if(isset($response['idProducto']) || isset($_POST['idProducto']))
    {
        ...
        $query="SELECT descripcion,descripcion_2,linea,acabado,material,calibre,capacidad,colores,anclaje,vaciado,largoTotal,anchoTotal,altoTotal,diametroTotal,largoCont,anchoCont,altoCont,diametroCont,largoLetrero,altoLetrero,adicional,imagen,url_ml,url_amz FROM descripcion_producto where id_descripcion = '$idProducto'";
        $result_query = $data_source->exeConsulta($query);
        foreach($result_query as $row)
        {
            $result[]=Array('descripcion'=>$row['descripcion'].' '.$row['descripcion_2'],'linea'=>$row['linea'],'acabado'=>$row['acabado'],'material'=>$row['material'],'calibre'=>$row['calibre'],'capacidad'=>$row['capacidad'],'colores'=>$row['colores'],'anclaje'=>$row['anclaje'],
                            'vaciado'=>$row['vaciado'],'largototal'=>$row['largoTotal'],'anchototal'=>$row['anchoTotal'],'altototal'=>$row['altoTotal'],'diametrototal'=>$row['diametroTotal'],'largocont'=>$row['largoCont'],'anchocont'=>$row['anchoCont'],'altocont'=>$row['altoCont'],
